    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/ha-nakim/eduapp.git'
            }
        }
        ...
        stage('Deploy and Service to Kubernetes') {
            steps {
                script {
                    // Kubernetes Deployment and Service 생성 및 적용 (1일차 교육때 사용한 deploy & service 생성 yaml 파일 등록하여 사용)
                    sh "kubectl apply -f ./yaml/edu_app.yaml -n ${NAMESPACE}"
                }
            }
        }
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment.apps/edu-app2 edu-app2=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
